...
class Particle(val emitter: Emitter)
{
	...
	companion object
	{
		...
		fun load(xml: XmlData, emitter: Emitter): Particle
		{
			...
			particle.blend = BlendMode.valueOf(xml.get("BlendMode", "Additive")!!.uppercase(Locale.ENGLISH))
			particle.collision = CollisionAction.valueOf(xml.get("Collision", "None")!!.uppercase(Locale.ENGLISH))
			...
			particle.sizeOrigin = SizeOrigin.valueOf(xml.get("SizeOrigin", "Center")!!.uppercase(Locale.ENGLISH))
			particle.sizeMode = SizeMode.valueOf(xml.get("SizeMode", "BothAxis")!!.uppercase(Locale.ENGLISH))
